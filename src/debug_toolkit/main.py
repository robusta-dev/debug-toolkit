...
@app.command()
def stack_trace(pid: int, all_threads: bool = True, amount: int = 1, sleep_duration_s=0, verbose: bool = False):
    if amount < 1 or sleep_duration < 0:
    ...
    payload = payload.replace("SLEEP_DURATION_S", str(sleep_duration_s))
    inject_string(pid, payload, trampoline=True, trampoline_timeout=10*amount+(sleep_duration_s+1), verbose=verbose)
